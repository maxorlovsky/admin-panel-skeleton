...
class LoginController
{
    ...
    public function login($attributes) {
        ...
        $q = $this->db->prepare('DELETE FROM `tm_users_auth_attempts` WHERE `ip` = :ip LIMIT 1');
    ...
    private function checkBruteForce() {
        $q = $this->db->prepare(
            'DELETE FROM `tm_users_auth_attempts` WHERE '.
            '`ip` = :ip AND '.
        ...
        $q = $this->db->prepare(
            'SELECT `attempts` FROM `tm_users_auth_attempts` WHERE '.
            '`ip` = :ip '.
            'LIMIT 1'
        );
        $q->bindParam(':ip', $_SERVER['REMOTE_ADDR'], PDO::PARAM_STR);
        ...
        if ($attempts) {
            ...
            $q = $this->db->prepare('UPDATE `tm_users_auth_attempts` SET '.
            ...
            // 1st attempt
            $q = $this->db->prepare('INSERT INTO `tm_users_auth_attempts` SET '.
                '`ip` = :ip, '.
                '`attempts` = 1'
            );
            $q->bindParam(':ip', $_SERVER['REMOTE_ADDR'], PDO::PARAM_STR);
    ...
    private function authentication($attributes) {
        $q = $this->db->prepare(
            'SELECT `id`, `password` FROM `tm_admins` WHERE `login` = :login LIMIT 1'
        );
    ...
    private function createToken($userId) {
        ...
        $this->db->query('DELETE FROM `tm_users_auth` WHERE `user_id` = '.(int)$userId.' LIMIT 1');
        $this->db->query('INSERT INTO `tm_users_auth` SET `user_id` = '.(int)$userId.', `token` = "'.$token.'"');
