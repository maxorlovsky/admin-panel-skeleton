...
class UsersController
{
    ...
    public function deleteAdmin($id) {
        ...
        $q = $this->db->prepare('UPDATE `tm_admins` SET `deleted` = 1 WHERE `id` = :id LIMIT 1');
    ...
    public function getAdmins() {
        $q = $this->db->prepare(
            'SELECT `id`, `login`, `email`, `level`, `last_login` FROM `tm_admins` '.
            'WHERE `deleted` = 0'
        );
        $q->execute();
