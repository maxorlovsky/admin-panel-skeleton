...
$app->put('/permissions', function(Request $request, Response $response) {
    if (!$request->getAttribute('isLogged')) {
        $response = $response->withStatus(401);
        $data = array('message' => 'Authorization required');
    } else {
...
class PermissionsController
{
    ...
    public function getPermissions() {
        $q = $this->db->query('SELECT `value` FROM `mo` WHERE `setting` = "menu" LIMIT 1');
        $q->execute();
        $row = $q->fetch();
    ...
    public function updatePermissions($attributes) {
        ...
        $q = $this->db->prepare('UPDATE `mo` SET `value` = :value WHERE `setting` = "menu"');
